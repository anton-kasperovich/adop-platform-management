  ...
  case "${PLATFORM_EXTENSION_TYPE}" in
      ...
      docker)
        ...
        if [ -f ${WORKSPACE}/service/${PLATFORM_EXTENSION_TYPE}/docker-compose.yml ] ; then
            ...
            echo $(docker exec proxy jq -r '.core[0].components |= .+ [{"id": "'$SERVICE_NAME'", "title": "'$PLATFORM_EXTENSION_NAME'", "img": "'$PLATFORM_EXTENSION_IMAGE'", "description": "'$PLATFORM_EXTENSION_DESCRIPTION'", "link": "", "linkCreate": {"host": "'$PLATFORM_EXTENSION_NAME'", "endPath": ""}}]' /usr/share/nginx/html/plugins.json) > plugins.json
